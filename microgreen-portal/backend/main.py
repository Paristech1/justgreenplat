...
def update_order_status(order_id: str, status_update: str):
    ...
    if status_update == "cancelled" and original_status != "cancelled":
    ...
    elif status_update == "confirmed" and original_status == "pending":
        print(f"Order {order_id} confirmed.")
        
    elif status_update == "completed" and original_status != "completed":
        print(f"Order {order_id} marked as completed.")

    orders[index]["status"] = status_update
...
def get_demand_forecast(weeks: int = 4):
    ...
    # Check cache first (simple example, would need better invalidation)
    # if cache_key in forecasts_cache:
    #    return Forecast(**forecasts_cache[cache_key])

    # --- Simple Forecasting Logic (Placeholder) ---
    # Calculate average daily demand per variety over the last 60 days
    historical_days = 60
    demand_per_variety_daily = defaultdict(float)
    total_days_considered = defaultdict(int)

    for order_dict in orders:
        if order_dict["status"] == "cancelled": continue
        order_dt = order_dict["orderDate"]
        if order_dt > datetime.now() - timedelta(days=historical_days):
            days_ago = (datetime.now() - order_dt).days
            for item_dict in order_dict["items"]:
                variety_name = item_dict["variety"]
                demand_per_variety_daily[variety_name] += item_dict["quantity"]
                # Track days where variety was ordered (simplistic)
                total_days_considered[variety_name] = max(total_days_considered[variety_name], days_ago)

    avg_demand_daily = { 
        variety: total_demand / historical_days 
        for variety, total_demand in demand_per_variety_daily.items()
        # Only include if ordered in the period
        # if total_days > 0 
    }
    
    # Project forward
    predictions = []
    total_predicted_trays_daily = sum(avg_demand_daily.values())
    
    for i in range(weeks * 7): # Predict for each day
        current_date = start_date + timedelta(days=i)
        # Simple: Assume average daily demand continues
        predicted_trays = total_predicted_trays_daily * (1 + random.uniform(-0.1, 0.1)) # Add noise
        predictions.append(ForecastPoint(date=current_date, predictedTrays=round(max(0, predicted_trays), 1)))
        
    # --- End Simple Forecasting Logic ---
    ...
    forecasts_cache[cache_key] = forecast_result.dict() # Cache the result
